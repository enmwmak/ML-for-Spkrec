...
y = zeros(max(spk_ids), size(v,1),'double');

if nargout > 1
  A = cell(size(N,2),1);  
  for c=1:size(N,2)
    A{c} = zeros(size(v,1),'double');
  end
  C = zeros(size(v,1), size(F,2),'double');
end

vEvT = cell(1,size(N,2));
for c=1:size(N,2)
...
for ii = unique(spk_ids)'
    fprintf('\rProcessing speaker %d of %d',ii,length(unique(spk_ids)));
    ...
    %invL = inv(L);
    %y(ii,:) = ((Fs ./ E) * v') * invL;
    y(ii,:) =  (L\((Fs ./ E) * v')')';             % E(y) = inv(l(s))*V'*inv(Sigma)*S_X(s) in Kenny05
    
    if nargout > 1
        condnum = cond(L);
        if condnum < 3000                            % 3000 means 13 decimal places precision for the inverse
            invL = inv(L);
            invL = invL + y(ii,:)' * y(ii,:);        % A_c in Kenney08; E(yy') in Kenny05
            for c=1:size(N,2)
                A{c} = A{c} + invL * Nss(c);           % sum_s N(s)E(yy') in Eq. 4 of Kenny05
            end
            C = C + y(ii,:)' * Fs;                   % sum_s S_X(s)E(y') in Eq. 4 of Kenny05
        else
            fprintf('Speaker %d has poor condition matrix L (cond=%f)\n', ii,condnum);
        end
    end
end
...
% plot the norm of i-vectors for debugging
%normy = zeros(size(y,1),1);
%for i = 1:size(y,1),
%    normy(i) = norm(y(i,:));
%end
%figure; plot(sort(normy));

% Print the range of norm of the basis vectors in v for debugging
%normv = zeros(size(v,1),1);
%for i = 1:size(v,1),
%    normv(i) = norm(v(i,:));
%end
%fprintf('Range of norm_v = %f\n',range(normv));
