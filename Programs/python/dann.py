...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 22 13:32:07 2019
@author: mwmak

Implementation of domain adversarial neural network (DANN). 
Derived from AdversarialTx.py
...
# A class containing an encoder and the whitening object for whitening data
class Encoder_wht:
    def __init__(self, encoder, wht_obj = None):
        self.encoder = encoder
        self.wht_obj = wht_obj
    ...
    def transform(self, X):
        return(self.wht_obj.transform(self.encoder.predict(X)))                

# The main DANN class containing encoder, discriminator and classifier            
class DANN(Transform):
    def __init__(self, savefunc=None, logdir='logs', f_dim=300, z_dim=100, n_cls=10, n_dom=2, 
                    enc_hnode=[512,512], cls_hnode=[512,512], dis_hnode=[64], whiten=False):
        ...
        self.n_classes = n_cls       # No. of classes (speakers) in the i/x-vectors
        ...
        #     But the objective function is not exact because -\sum_i(1-t_i)log y_i is not equal to
        #     \sum_i t_i * log y_i
        # (2) A better way is to define our own loss function, we have cross-entropy as usual for
        #     classifier's loss but the negative of the cross-entropy for discriminator loss. 
        #     But the loss and accuracy returned from train_on_batch are different from option 1
    ...
    # The generator comprises a feature encoder G(x) and a speaker classifier C(G(x)
    ...
    def fit(self, x_trn, y_spk_trn_1h, y_dom_trn_1h, n_epochs=20, batch_size=128):
        ...
        if (self.whiten is True):
            spk_lbs = np.argmax(y_spk_trn_1h, axis=1)
            self.wht_obj = WCCN().fit(self.encoder.predict(x_trn), spk_lbs)
        else:
            self.wht_obj = None
            
        # Create an object comprising encoder and whitening object    
        self.encoder_wht = Encoder_wht(self.encoder, self.wht_obj)        
    ...
    def transform(self, X):
        ...
        if self.whiten is True:
            return self.wht_obj.transform(Y)
        else:
            return Y

    # Save an object comprising an encoder and a whitening object
    def save_encoder_wht(self, filename):
        self.encoder_wht.save(filename)


    # Save the whole DANN model, include encoder, discriminator and classifier
    def save_model(self, filename):
        with open(filename, 'wb') as f:  # Overwrites any existing file.
            pickle.dump(self, f, pickle.HIGHEST_PROTOCOL)

        
    # Load the whole DANN model, include encoder, discriminator and classifier    
    @staticmethod        
