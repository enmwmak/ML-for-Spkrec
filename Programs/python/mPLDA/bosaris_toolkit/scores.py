...
class Scores:
    ...
    def get_tar_non(self, key):
        ...
        knnonndx = key.knnon & new_score.scoremask
        uknnonndx = key.uknnon & new_score.scoremask
        tar = new_score.scoremat[tarndx]
        non = new_score.scoremat[nonndx]
        knnon = (new_score.scoremat[knnonndx])
        uknnon = (new_score.scoremat[uknnonndx])

        return tar, non, knnon, uknnon
    ...
    def read_evl(cls, input_file_name):
        """Creates a Scores object from information stored in a text file.
        ...
        for ii in range(len(lines)):
            models[ii] = lines[ii][0]
            testsegs[ii] = lines[ii][1] + ':' + lines[ii][2]
            scores[ii] = float(lines[ii][3])
