...
class Key:
    ...
    def __init__(self, key_file_name='',
        ...
        self.knnon = numpy.array([], dtype="bool")
        self.uknnon = numpy.array([], dtype="bool")
        ...
        if key_file_name == '':
            ...
            self.tar = tmp.tar
            self.non = tmp.non
            self.knnon = tmp.knnon
            self.uknnon = tmp.uknnon
    ...
    def filter(self, modlist, seglist, keep):
        ...
        # modified
        tmp = self.knnon[numpy.array(keepmodidx), :]
        outkey.knnon = tmp[:, numpy.array(keepsegidx)]
        tmp = self.uknnon[numpy.array(keepmodidx), :]
        outkey.uknnon = tmp[:, numpy.array(keepsegidx)]
    ...
    def read(input_file_fame):
        ...
        with h5py.File(input_file_fame, "r") as f:
            ...
            # modified
            key.tar = f.get("tar")[()].astype(numpy.bool)
            key.non = f.get("non")[()].astype(numpy.bool)
            key.knnon = f.get("knnon")[()].astype(numpy.bool)
            key.uknnon = f.get("uknnon")[()].astype(numpy.bool)
